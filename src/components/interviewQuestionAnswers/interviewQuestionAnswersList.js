...
const interviewQuestionAnswersList = ({...props}) => {
  const {
    lookupTableAllUsers,
    intQuestionAnswers,
  ...
  let allAnswersArray = UtilityMethods.getArrayOfObjectsByKey(lookupTableAllInterviewQuestionAnswers, intQuestionAnswers)
  ...
  if(allAnswersArray.length === 0){
    ...
    let allAnswers = allAnswersArray.map(answer => {
      ...
      if(lookupTableAllUsers[answer[_createdByUser]]){
        answerWrittenByUser = lookupTableAllUsers[answer[_createdByUser]].name
      }
...
export default interviewQuestionAnswersList;